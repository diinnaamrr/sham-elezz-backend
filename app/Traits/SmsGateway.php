        ...
        if (isset($config) && $config['status'] == 1) {
            ...
            try {
                $res = Http::withHeaders([
                    'Accept' => 'application/json',
                    'Content-Type' => 'application/json',
                ])->post('https://bulk.whysms.com/api/http/sms/send', $payload);

                if ($res->successful()) {
                    $response = 'success';
                    Log::info('WhySMS send success (trait)', [
                        'recipient' => $recipient,
                        'status' => $res->status(),
                        'body' => $res->body(),
                    ]);
                } else {
                    Log::error('WhySMS send failed (trait)', [
                        'recipient' => $recipient,
                        'status' => $res->status(),
                        'body' => $res->body(),
                    ]);
                ...
                Log::error('WhySMS send exception (trait)', [
