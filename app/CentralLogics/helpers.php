...
class Helpers
{
                                                        ...
                                                        $selected_addon_quantity,$trans = false, $local = 'en')
    {
        ...
        // استخدم المتجر الفعلي والرصيد الفعلي (لدعم المنيو المشترك)
        list($effectiveStore, $effectiveStock) = self::effective_store_and_stock($data);

        // منيو مشترك: لو store_id على الـ item نفسه null، رجّعه من الـ context_store_id
        $contextStoreId = $data->getAttribute('context_store_id');
        if (($data->is_shared_menu ?? false) && $contextStoreId && empty($data['store_id'])) {
            $data['store_id'] = (int) $contextStoreId;
        }

        $data['store_name'] = $effectiveStore?->name;
        $data['is_campaign'] = $effectiveStore?->campaigns_count > 0 ? 1 : 0;
        $data['module_type'] = $data->module->module_type;
        $data['zone_id'] = $effectiveStore?->zone_id;
        ...
        $data['stock'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0))
            ? $running_flash_sale->available_stock
            : $effectiveStock;
        ...
        $data['store_discount'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0))
            ? 0
            : (self::get_store_discount($effectiveStore) ? $effectiveStore?->discount->discount : 0);
        $data['schedule_order'] = $effectiveStore?->schedule_order;
        ...
        $data['min_delivery_time'] = $effectiveStore && $effectiveStore->delivery_time
            ? (int) (explode('-', $effectiveStore->delivery_time)[0] ?? 0)
            : 0;
        $data['max_delivery_time'] = $effectiveStore && $effectiveStore->delivery_time
            ? (int) (explode('-', $effectiveStore->delivery_time)[1] ?? 0)
            : 0;
        ...
        $data['halal_tag_status'] = (int) ($effectiveStore?->storeConfig?->halal_tag_status ?? 0);
    ...
    /** منيو مشترك: استخراج المتجر والرصيد الفعليين حسب context_store_id إن وُجد */
    protected static function effective_store_and_stock($item)
    {
        $contextStoreId = $item->getAttribute('context_store_id');
        $isShared = $item->is_shared_menu && $contextStoreId;
        $store = $isShared ? Store::find($contextStoreId) : $item->store;
        $stock = $isShared ? $item->getStockForStore($contextStoreId) : (int)($item->stock ?? 0);
        return [$store, $stock];
    }
    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en' , $temp_product=false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                list($effectiveStore, $effectiveStock) = self::effective_store_and_stock($item);
                $variations = [];

                // منيو مشترك: لو store_id على الـ item نفسه null، رجّعه من الـ context_store_id
                $contextStoreId = $item->getAttribute('context_store_id');
                if (($item->is_shared_menu ?? false) && $contextStoreId && empty($item['store_id'])) {
                    $item['store_id'] = (int) $contextStoreId;
                }
                ...
                $item['store_name'] = $effectiveStore?->name;
                $item['is_campaign'] = $effectiveStore?->campaigns_count>0?1:0;
                $item['zone_id'] = $effectiveStore?->zone_id;
                $running_flash_sale = FlashSaleItem::Active()->whereHas('flashSale', function ($query) {
                    $query->Active()->Running();
                })
                    ->where(['item_id' => $item['id']])->first();
                $item['flash_sale'] =(int) ((($running_flash_sale && ($running_flash_sale->available_stock > 0)) ? 1 :0));
                $item['stock'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0)) ? $running_flash_sale->available_stock : $effectiveStock;
                ...
                $item['store_discount'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0)) ? 0 : (self::get_store_discount($effectiveStore) ? $effectiveStore?->discount->discount : 0);
                $item['schedule_order'] = $effectiveStore?->schedule_order;
                $item['delivery_time'] = $effectiveStore?->delivery_time;
                $item['free_delivery'] = $effectiveStore?->free_delivery;
                $item['tax'] = $effectiveStore?->tax;
                $item['unit'] = $item->unit;
                ...
                $item['recommended'] =(int) $item->recommended;
                $item['min_delivery_time'] =  (int) explode('-',$effectiveStore?->delivery_time ?? '-')[0] ?? 0;
                $item['max_delivery_time'] =  (int) explode('-',$effectiveStore?->delivery_time ?? '-')[1] ?? 0;
                ...
                $item['halal_tag_status'] =  (int) $effectiveStore?->storeConfig?->halal_tag_status??0;

                $item['self_delivery_system'] = (int) ($effectiveStore?->sub_self_delivery ?? 0);
                if ($item->store !== null) {
                    $item->store['self_delivery_system'] = (int) ($effectiveStore?->sub_self_delivery ?? 0);
                }
            ...
            $running_flash_sale = FlashSaleItem::Active()->whereHas('flashSale', function ($query) {
                ...
                ->where(['item_id' => $data['id']])->first();
            $data['flash_sale'] =(int) (($running_flash_sale) ? 1 :0);
            $data['stock'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0)) ? $running_flash_sale->available_stock : $effectiveStock;
            $data['discount'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0)) ? $running_flash_sale->discount : $data['discount'];
            $data['discount_type'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0)) ? $running_flash_sale->discount_type : $data['discount_type'];
            $data['store_discount'] = ($running_flash_sale && ($running_flash_sale->available_stock > 0)) ? 0 : (self::get_store_discount($effectiveStore) ? $effectiveStore?->discount->discount : 0);
            $data['schedule_order'] = $effectiveStore?->schedule_order;
            $data['rating_count'] = (int)($data->rating ? array_sum(json_decode($data->rating, true)) : 0);
            $data['avg_rating'] = (float)($data->avg_rating ? $data->avg_rating : 0);
            $data['min_delivery_time'] =  (int) explode('-',$effectiveStore?->delivery_time ?? '-')[0] ?? 0;
            $data['max_delivery_time'] =  (int) explode('-',$effectiveStore?->delivery_time ?? '-')[1] ?? 0;
            $data['common_condition_id'] =  (int) $data->pharmacy_item_details?->common_condition_id ?? 0;
            $data['brand_id'] =  (int) $data->ecommerce_item_details?->brand_id ?? 0;
            $data['is_basic'] =  (int) $data->pharmacy_item_details?->is_basic ?? 0;
            $data['is_prescription_required'] =  (int) $data->pharmacy_item_details?->is_prescription_required ?? 0;
            ...
            if ($effectiveStore) {
                ...
                $data->store['self_delivery_system'] = (int) $effectiveStore->sub_self_delivery;
            }
