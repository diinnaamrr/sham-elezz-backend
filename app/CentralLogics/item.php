...
class ProductLogic
{
    /** حلّ store_id (رقم أو slug) إلى id للمستخدم في استعلامات المنيو المشترك */
    protected static function resolveStoreId($store_id): ?int
    {
    ...
    public static function get_latest_products($zone_id, $limit, $offset, $store_id, $category_id, $type, $min=false, $max=false, $product_id=null)
    {
        ...
        ->where(function($query) use ($zone_id) {
            $query->whereHas('store', function($q) use($zone_id){
                $q->when(config('module.current_module_data'), function($q){
                    $q->where('module_id', config('module.current_module_data')['id'])->whereHas('zone.modules',function($q){
                        $q->where('modules.id', config('module.current_module_data')['id']);
                    });
                })->whereIn('zone_id', json_decode($zone_id, true));
            })->orWhere('items.is_shared_menu', true);
        })
        ...
        ->select(['items.*'])
        ->selectSub(function ($subQuery) use ($store_id) {
            $resolved = (int) (self::resolveStoreId($store_id) ?? 0);
            $subQuery->selectRaw('active as temp_available')
                ->from('stores')
                ->whereRaw('stores.id = COALESCE(items.store_id, ?)', [$resolved]);
        }, 'temp_available')
        ->active()->type($type);
        ...
        if ($latest_items_default_status == '1'){
            ...
            if(config('module.current_module_data')['module_type']  !== 'food'){
                $resolvedStore = self::resolveStoreId($store_id);
                if($latest_items_sort_by_unavailable == 'remove'){
                    $query = $query->inStockForStore($resolvedStore);
                }elseif($latest_items_sort_by_unavailable == 'last'){
                    $query = $query->orderByRaw('CASE WHEN COALESCE(items.is_shared_menu, 0) = 0 THEN CASE WHEN items.stock = 0 THEN 1 ELSE 0 END ELSE CASE WHEN (SELECT COALESCE(stock,0) FROM store_item_stock WHERE item_id = items.id AND store_id = ' . (int)($resolvedStore ?? 0) . ' LIMIT 1) <= 0 THEN 1 ELSE 0 END END');
                }
    ...
    public static function update_stock($item, $quantity, $variant=null, $storeId=null)
    {
        if(isset($variant))
        {
        ...
        // أولاً: لو في سجل في store_item_stock للـ item + store ده، اعتبره مصدر الحقيقة للمخزون
        if ($storeId) {
            $stockRow = \App\Models\StoreItemStock::where('item_id', $item->id)
                ->where('store_id', $storeId)
                ->first();
            if ($stockRow) {
                $stockRow->stock = max(0, $stockRow->stock - $quantity);
                $stockRow->save();
            }
            return $item;
        }

        // fallback: مخزون عام على مستوى الـ item (للمنتجات غير المرتبطة بمخزون متجر محدد)
        $item->stock = max(0, ($item->stock ?? 0) - $quantity);
        return $item;
    }
