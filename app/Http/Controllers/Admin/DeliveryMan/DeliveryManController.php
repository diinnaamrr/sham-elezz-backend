...
class DeliveryManController extends BaseController
{
   ...
   public function add(DeliveryManAddRequest $request): RedirectResponse
{
    $data = $this->deliveryManService->getAddData(request: $request);
    
    $data['zone_id'] = intval($request->input('zone_id')); // ✅ Ensures it's an integer


    $this->deliveryManRepo->add(data: $data);
