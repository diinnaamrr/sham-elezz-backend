...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'sku' => 'nullable|string|unique:items,sku|max:255',
    ...
    'nullable',
    Rule::when($request->hasFile('image'), [  // If it's a file, validate as an image
        'image',
        'mimes:jpg,png,jpeg,gif,webp',
        'max:2048'
    ], [  // If it's not a file, validate as a URL
        'string',
        'max:255',
...
if ($request->has('image') && filter_var($request->image, FILTER_VALIDATE_URL)) {
    ...
    $item->image = $request->hasFile('image') ? Helpers::upload('product/', 'png', $request->file('image')) : ($newFileNamethumb ?? null);
        ...
        $is_shared_menu = (bool) $request->is_shared_menu;
        $item->is_shared_menu = $is_shared_menu;
        if ($is_shared_menu) {
            $item->store_id = null;
            $item->stock = 0;
        } else {
        ...
        $item->production_date = $request->production_date ?? null;
        $item->expiration_date = $request->expiration_date ?? null;
        $item->save();
        if ($is_shared_menu && $request->has('shared_menu_default_stock')) {
            $default_stock = (int) $request->shared_menu_default_stock;
            $storeIds = Store::where('module_id', Config::get('module.current_module_id'))->pluck('id');
    ...
    public function edit(Request $request, $id)
{
    ...
    if ($request->temp_product) {
        ...
        $product = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->with('store', 'category', 'module', 'storeStocks')->findOrFail($id);
    }
    ...
    public function update(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'cost' => 'nullable|numeric|min:0', // Ensure cost is a valid number
            'name' => 'array',
            'name.0' => 'required',
            'name.*' => 'max:191',
            'category_id' => 'required',
           ...
           'production_date' => 'nullable|date',
            ...
            'sku.unique' => translate('messages.sku_must_be_unique'),
            'description.*.max' => translate('messages.description_length_warning'),
            'category_id.required' => translate('messages.category_required'),
            'name.0.required' => translate('default_name_is_required'),
            'description.0.required' => translate('default_description_is_required'),
        ...
        $item = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->find($id);
        
        // حفظ البيانات القديمة للبحث عن المنتجات المتشابهة
        $old_name = $item->getAttributes()['name'] ?? $item->name; // الحصول على الاسم من الحقل مباشرة
        $old_category_id = $item->category_id;
        $old_sku = $item->sku; // حفظ SKU القديم
...
if ($request->has('image') && filter_var($request->image, FILTER_VALIDATE_URL)) {
    ...
    $item->image = $request->hasFile('image') ? Helpers::update('product/', $item->image, 'png', $request->file('image')) : $item->image;
}
        $item->available_time_starts = $request->available_time_starts ?? '00:00:00';
        $item->available_time_ends = $request->available_time_ends ?? '23:59:59';
        $item->sku = $request->sku; 
      $item->cost = $request->has('cost') ? (float) $request->cost : $item->cost;
        ...
        $item->is_halal = $request->is_halal ?? 0;
        $item->organic = $request->organic ?? 0;
          // Handle production_date and expiration_date - ensure they are saved properly
        if ($request->has('production_date') && !empty(trim($request->production_date))) {
            $item->production_date = $request->production_date;
        } else {
            $item->production_date = null;
        }
        if ($request->has('expiration_date') && !empty(trim($request->expiration_date))) {
        ...
        if ($is_shared_menu && $request->has('shared_menu_default_stock')) {
            $default_stock = (int) $request->shared_menu_default_stock;
            StoreItemStock::where('item_id', $item->id)->delete();
            $storeIds = Store::where('module_id', Config::get('module.current_module_id'))->pluck('id');
            foreach ($storeIds as $storeId) {
        ...
        // مزامنة اسم اللغة الحالية مع الاسم الافتراضي حتى يظهر الاسم المحدث في القوائم (أدمن/متجر)
        Translation::updateOrCreate(
            [
                'translationable_type' => Item::class,
                'translationable_id' => $item->id,
        ...
        // البحث عن المنتجات المتشابهة في المحلات الأخرى
        // نستخدم استراتيجية ذكية: نبحث عن جميع المنتجات التي قد تكون نفس المنتج
        // 1. البحث بالـ SKU (إذا كان متشابه)
        // 2. البحث بالاسم القديم + الفئة
        // 3. البحث بالاسم الجديد + الفئة
        // 4. البحث عن جميع المنتجات في نفس الفئة التي لها نفس الاسم (بغض النظر عن التغييرات السابقة)
        $similar_items = collect();
        
        // الطريقة 1: البحث بالـ SKU (إذا كان SKU متشابه، فهي نفس المنتج)
        if (!empty($old_sku) && !empty($request->sku) && $old_sku == $request->sku) {
            $similar_items_sku = Item::withoutGlobalScope(StoreScope::class)
                ->withoutGlobalScope('translate')
                ->where('id', '!=', $item->id)
                ->where('sku', $old_sku)
                ->get();
            
            $similar_items = $similar_items->merge($similar_items_sku);
        }
        
        // الطريقة 2: البحث عن المنتجات التي لها نفس الاسم القديم ونفس الفئة القديمة
        $similar_items_old = Item::withoutGlobalScope(StoreScope::class)
            ->withoutGlobalScope('translate')
            ->where('id', '!=', $item->id)
            ->whereRaw('LOWER(TRIM(name)) = ?', [strtolower(trim($old_name))])
            ->where('category_id', $old_category_id)
            ->get();
        
        $similar_items = $similar_items->merge($similar_items_old);
        
        // الطريقة 3: البحث عن المنتجات التي لها نفس الاسم الجديد ونفس الفئة الجديدة
        $similar_items_new = Item::withoutGlobalScope(StoreScope::class)
            ->withoutGlobalScope('translate')
            ->where('id', '!=', $item->id)
        ...
        // الطريقة 4: البحث عن جميع المنتجات في نفس الفئة التي لها نفس الاسم (بغض النظر عن التغييرات)
        // هذا مهم للتأكد من ربط جميع المنتجات المتشابهة حتى لو تغير الاسم عدة مرات
        // نبحث عن المنتجات التي لها نفس الفئة واسمها مطابق للاسم القديم أو الجديد
        ...
        $similar_items = $similar_items->merge($all_similar_in_category);
        
        // الطريقة 5: البحث في جدول الترجمات أيضاً (للتأكد من العثور على جميع المنتجات المتشابهة)
        // نبحث عن المنتجات التي لها نفس الفئة واسمها في الترجمات مطابق للاسم القديم أو الجديد
        $translation_items = DB::table('items')
            ->join('translations', function($join) {
                $join->on('items.id', '=', 'translations.translationable_id')
                     ->where('translations.translationable_type', '=', 'App\Models\Item')
        ...
        // تحويل النتائج إلى Collection من Item models
        $translation_items_models = Item::withoutGlobalScope(StoreScope::class)
            ->withoutGlobalScope('translate')
            ->whereIn('id', $translation_items->pluck('id'))
            ->get();
        
        $similar_items = $similar_items->merge($translation_items_models);
        
        // إزالة التكرارات
        $similar_items = $similar_items->unique('id');
        
        // تسجيل للتحقق من النتائج (يمكن حذفه لاحقاً)
        \Log::info('Product Update - Similar Items Found', [
            'old_name' => $old_name,
            'new_name' => $new_name,
        ...
        // تحديث المنتجات المتشابهة في المحلات الأخرى
        foreach ($similar_items as $similar_item) {
            // تحديث الحقول المشتركة فقط (لا نحدث store_id أو stock أو status)
            $similar_item->name = $new_name;
            $similar_item->description = $item->description;
            $similar_item->image = $item->image;
            $similar_item->images = $item->images;
            $similar_item->price = $item->price;
            $similar_item->cost = $item->cost; // تحديث cost أيضاً
            $similar_item->discount = $item->discount;
            $similar_item->discount_type = $item->discount_type;
            $similar_item->category_id = $item->category_id;
            $similar_item->category_ids = $item->category_ids;
            $similar_item->variations = $item->variations;
            $similar_item->food_variations = $item->food_variations;
            $similar_item->choice_options = $item->choice_options;
            $similar_item->attributes = $item->attributes;
            $similar_item->add_ons = $item->add_ons;
            $similar_item->veg = $item->veg;
            $similar_item->organic = $item->organic;
            $similar_item->is_halal = $item->is_halal;
            $similar_item->production_date = $item->production_date;
            $similar_item->expiration_date = $item->expiration_date;
            $similar_item->unit_id = $item->unit_id;
            $similar_item->maximum_cart_quantity = $item->maximum_cart_quantity;
            $similar_item->available_time_starts = $item->available_time_starts;
            $similar_item->available_time_ends = $item->available_time_ends;
            
            // تحديث slug حسب الاسم الجديد
            // ننشئ slug جديد من الاسم الجديد ونضيف id المنتج لضمان التفرد
            $new_slug_base = Str::slug($new_name);
            $similar_item->slug = "{$new_slug_base}-{$similar_item->id}";
            
            $similar_item->save();
            
            // تحديث الترجمات للمنتجات المتشابهة - مهم جداً لأن الداش بورد يعرض الاسم من الترجمات
            // نحذف الترجمات القديمة أولاً ثم نضيف الجديدة
            \App\Models\Translation::where('translationable_type', 'App\Models\Item')
                ->where('translationable_id', $similar_item->id)
                ->whereIn('key', ['name', 'description'])
                ->delete();
            
            // إضافة الترجمات الجديدة بنفس طريقة المنتج الأصلي
            Helpers::add_or_update_translations(request: $request, key_data: 'name', name_field: 'name', model_name: 'Item', data_id: $similar_item->id, data_value: $new_name);
            Helpers::add_or_update_translations(request: $request, key_data: 'description', name_field: 'description', model_name: 'Item', data_id: $similar_item->id, data_value: $item->description);
            
            // تحديث العلاقات (tags, nutritions, allergies, generic)
            $similar_item->tags()->sync($tag_ids);
            $similar_item->nutritions()->sync($nutrition_ids);
            $similar_item->allergies()->sync($allergy_ids);
            
            if($similar_item->module && $similar_item->module->module_type == 'pharmacy'){
                $similar_item->generic()->sync($generic_ids);
            }
        }
        ...
        if ($request->button == 'import') {
            ...
            try{
                foreach ($collections as $collection) {
                  ...
                  $image = isset($collection['Image']) ? trim($collection['Image']) : null;

if (!empty($image) && !filter_var($image, FILTER_VALIDATE_URL)) {
    // If the image is not a URL, assume it's a local file
    $image = 'uploads/' . $image;
}
                    array_push($data, [
                        'name' => $collection['Name'],
                        'description' => $collection['Description'],
    'image' => $image, // Use processed image
                        'images' => $collection['Images'] ?? json_encode([]),
                        // Commented out: Sub-category support disabled - only main categories allowed
                        'category_id' => $collection['CategoryId'], // Always use main category
                        'category_ids' => json_encode([['id' => $collection['CategoryId'], 'position' => 0]]), // Only main category
						...
						'sku' => $collection['SKU'],
                         'cost' => isset($collection['Cost']) ? (float) $collection['Cost'] : 0, // Add cost column
...
//                DB::table('items')->upsert($chunk_item, ['id', 'module_id'], ['name', 'description', 'image', 'images', 'category_id', 'category_ids', 'unit_id', 'stock', 'price', 'discount', 'discount_type', 'available_time_starts', 'available_time_ends','choice_options', 'variations', 'food_variations', 'add_ons', 'attributes', 'store_id', 'status', 'veg', 'recommended']);
    ...
    public function bulk_export_data(Request $request)
    {
        ...
        $products = Item::when($request['type'] == 'date_wise', function ($query) use ($request) {
            ...
            // Commented out: module filtering disabled - only categories and items allowed
            // ->module(Config::get('module.current_module_id'))
...
// dd($request?->sub_tab,$foods,);
    ...
    public function approved(Request $request)
    {
       ...
       $item->production_date = $data->production_date ?? null;
        $item->expiration_date = $data->expiration_date ?? null;
