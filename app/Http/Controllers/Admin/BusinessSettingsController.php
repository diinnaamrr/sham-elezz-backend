...
class BusinessSettingsController extends Controller
{
    ...
    public function update_dm(Request $request)
    {
        ...
        Helpers::businessUpdateOrInsert(['key' => 'show_dm_total_earning'], [
            'value' => $request['show_dm_total_earning'] ?? 0
        ]);

        Helpers::businessUpdateOrInsert(['key' => 'dm_performance_tracking'], [
            'value' => $request['dm_performance_tracking'] ?? 0
        ]);
...
// dd( $request['commission_business_model']);
    ...
    public function update_app_settings(Request $request)
    {
        ...
        $request->validate([
            'app_minimum_version_android' => 'nullable|regex:/^\d\.\d\.\d$/',
            'app_minimum_version_ios' => 'nullable|regex:/^\d\.\d\.\d$/',
            'app_minimum_version_android_store' => 'nullable|regex:/^\d\.\d\.\d$/',
            'app_minimum_version_ios_store' => 'nullable|regex:/^\d\.\d\.\d$/',
            'app_minimum_version_android_deliveryman' => 'nullable|regex:/^\d\.\d\.\d$/',
            'app_minimum_version_ios_deliveryman' => 'nullable|regex:/^\d\.\d\.\d$/',
            'user_android_latest_version' => 'nullable|regex:/^\d\.\d\.\d$/',
            'user_ios_latest_version' => 'nullable|regex:/^\d\.\d\.\d$/',
            'store_android_latest_version' => 'nullable|regex:/^\d\.\d\.\d$/',
            'store_ios_latest_version' => 'nullable|regex:/^\d\.\d\.\d$/',
            'deliveryman_android_latest_version' => 'nullable|regex:/^\d\.\d\.\d$/',
            'deliveryman_ios_latest_version' => 'nullable|regex:/^\d\.\d\.\d$/',
            'user_android_blocked_versions' => [
                'nullable',
                function ($attribute, $value, $fail) {
                    $versions = array_filter(preg_split('/[\s,]+/', $value));
                    foreach ($versions as $version) {
                        if (!preg_match('/^\d\.\d\.\d$/', $version)) {
                            $fail(translate('messages.format_is_invalid_for_') . ' ' . $version . '. ' . translate('messages.each_part_must_be_a_single_digit_(eg: 1.2.3)'));
                        }
                    }
                },
            ],
            'user_ios_blocked_versions' => [
                'nullable',
                function ($attribute, $value, $fail) {
                    $versions = array_filter(preg_split('/[\s,]+/', $value));
                    foreach ($versions as $version) {
            ...
            'store_android_blocked_versions' => [
                'nullable',
                function ($attribute, $value, $fail) {
                    $versions = array_filter(preg_split('/[\s,]+/', $value));
                    foreach ($versions as $version) {
            ...
            'deliveryman_android_blocked_versions' => [
                'nullable',
                function ($attribute, $value, $fail) {
                    $versions = array_filter(preg_split('/[\s,]+/', $value));
                    foreach ($versions as $version) {
            ...
            'deliveryman_ios_blocked_versions' => [
                'nullable',
                function ($attribute, $value, $fail) {
                    $versions = array_filter(preg_split('/[\s,]+/', $value));
                    foreach ($versions as $version) {
                        if (!preg_match('/^\d\.\d\.\d$/', $version)) {
                            $fail(translate('messages.blocked_versions_format_is_invalid_for_') . ' ' . $version);
                        }
                    }
                },
            ],
        ], [
            'regex' => translate('messages.version_must_be_in_X.Y.Z_format_where_each_is_a_single_digit_(eg: 1.2.3)')
        ]);
        ...
        if ($request->type == 'user_app') {
            ...
            Helpers::businessUpdateOrInsert(['key' => 'user_app_maintenance_mode'], [
                'value' => $request['user_app_maintenance_mode']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'user_app_maintenance_message'], [
                'value' => $request['user_app_maintenance_message']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'user_android_latest_version'], [
                'value' => $request['user_android_latest_version']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'user_ios_latest_version'], [
                'value' => $request['user_ios_latest_version']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'user_android_force_update'], [
                'value' => $request['user_android_force_update']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'user_ios_force_update'], [
                'value' => $request['user_ios_force_update']
            ]);

            $blocked_android = array_filter(preg_split('/[\s,]+/', $request['user_android_blocked_versions']));
            Helpers::businessUpdateOrInsert(['key' => 'user_android_blocked_versions'], [
                'value' => json_encode(array_values($blocked_android))
            ]);

            $blocked_ios = array_filter(preg_split('/[\s,]+/', $request['user_ios_blocked_versions']));
            Helpers::businessUpdateOrInsert(['key' => 'user_ios_blocked_versions'], [
                'value' => json_encode(array_values($blocked_ios))
            ]);
        ...
        if ($request->type == 'store_app') {
            ...
            Helpers::businessUpdateOrInsert(['key' => 'store_app_maintenance_mode'], [
                'value' => $request['store_app_maintenance_mode']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'store_app_maintenance_message'], [
                'value' => $request['store_app_maintenance_message']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'store_android_latest_version'], [
                'value' => $request['store_android_latest_version']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'store_ios_latest_version'], [
                'value' => $request['store_ios_latest_version']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'store_android_force_update'], [
                'value' => $request['store_android_force_update']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'store_ios_force_update'], [
                'value' => $request['store_ios_force_update']
            ]);

            $blocked_android = array_filter(preg_split('/[\s,]+/', $request['store_android_blocked_versions']));
            Helpers::businessUpdateOrInsert(['key' => 'store_android_blocked_versions'], [
                'value' => json_encode(array_values($blocked_android))
            ]);

            $blocked_ios = array_filter(preg_split('/[\s,]+/', $request['store_ios_blocked_versions']));
            Helpers::businessUpdateOrInsert(['key' => 'store_ios_blocked_versions'], [
                'value' => json_encode(array_values($blocked_ios))
            ]);
        ...
        if ($request->type == 'deliveryman_app') {
            ...
            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_app_maintenance_mode'], [
                'value' => $request['deliveryman_app_maintenance_mode']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_app_maintenance_message'], [
                'value' => $request['deliveryman_app_maintenance_message']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_android_latest_version'], [
                'value' => $request['deliveryman_android_latest_version']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_ios_latest_version'], [
                'value' => $request['deliveryman_ios_latest_version']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_android_force_update'], [
                'value' => $request['deliveryman_android_force_update']
            ]);

            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_ios_force_update'], [
                'value' => $request['deliveryman_ios_force_update']
            ]);

            $blocked_android = array_filter(preg_split('/[\s,]+/', $request['deliveryman_android_blocked_versions']));
            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_android_blocked_versions'], [
                'value' => json_encode(array_values($blocked_android))
            ]);

            $blocked_ios = array_filter(preg_split('/[\s,]+/', $request['deliveryman_ios_blocked_versions']));
            Helpers::businessUpdateOrInsert(['key' => 'deliveryman_ios_blocked_versions'], [
                'value' => json_encode(array_values($blocked_ios))
            ]);
...
//        ]);
    ...
    {
        // Clear the cache before fetching the settings to get the latest values
