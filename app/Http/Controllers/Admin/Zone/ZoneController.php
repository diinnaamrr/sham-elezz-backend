...
class ZoneController extends BaseController
{
       ...
       public function updateModuleSetup(ZoneModuleUpdateRequest $request, $id): RedirectResponse
    {
        ...
        $data = $this->zoneService->getZoneModuleSetupData(request: $request);

        $moduleData = collect($request->module_data)->map(function ($module) {
            return [
                'per_km_shipping_charge' => $module['per_km_shipping_charge'] ?? 0,
                'minimum_shipping_charge' => $module['minimum_shipping_charge'] ?? 0,
                'maximum_shipping_charge' => $module['maximum_shipping_charge'] ?? 0,
                'maximum_cod_order_amount' => $module['maximum_cod_order_amount'] ?? null,
        
                 'base_distance_km' => $module['base_distance_km'] ?? 0,
                'base_fee' => $module['base_fee'] ?? 0,
                'extra_price_per_km' => $module['extra_price_per_km'] ?? 0,
            ];
        })->toArray();
        
        $this->zoneRepo->zoneModuleSetupUpdate(id: $id, data: $data, moduleData: $moduleData);
