...
class CategoryController extends BaseController
{
    ...
    {
        // Commented out: Sub category support disabled - only main categories (position 0)
        $categories = $this->categoryRepo->getListWhere(
            searchValue: $request['search'],
            filters: ['position' => 0], // Always 0 - only main categories
            // relations: ['module'], // Commented out: Module relation not needed
        ...
        return view($this->categoryService->getViewByPosition(0), compact('categories','language','defaultLang','mainCategories')); // Always position 0
    }
    ...
    public function add(CategoryAddRequest $request): RedirectResponse
    {
        ...
        $category = $this->categoryRepo->add(
            data: $this->categoryService->getAddData(
                request: $request,
                parentCategory: null // No parent categories
            )
        );
        $this->translationRepo->addByModel(request: $request, model: $category, modelPath: 'App\Models\Category', attribute: 'name');
        Toastr::success(translate('messages.category_added_successfully')); // Always main category message
        return back();
    }
