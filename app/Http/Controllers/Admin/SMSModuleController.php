...
class SMSModuleController extends Controller
{
    ...
    public function sms_index()
    {
        ...
        foreach ($this->defaultGateways as $gateway => $payload) {
        ...
        $data_values = Setting::where('settings_type', 'sms_config')
            ->whereIn('key_name', array_keys($this->defaultGateways))
            ->get() ?? [];

        $data_values->transform(function ($setting) {
            $defaultValues = $this->defaultGateways[$setting->key_name] ?? ['status' => 0];
            $liveValues = is_array($setting->live_values) ? $setting->live_values : [];
            $setting->live_values = array_merge($defaultValues, $liveValues);
            return $setting;
        });
    ...
    public function sms_update(Request $request, $module)
    {
        ...
        } elseif ($module == 'whysms') {
            $additional_data = [
                'status' => $request['status'],
                'api_token' => $request['api_token'],
                'sender_id' => $request['sender_id'] ?? 'WhySMS Test',
                'otp_template' => $request['otp_template'] ?? null,
            ];
        }
        ...
        DB::table('addon_settings')->updateOrInsert(
            ['key_name' => $module, 'settings_type' => 'sms_config'],
            [
                'key_name' => $module,
        ...
        if ($request['status'] == 1) {
            foreach (['twilio', 'nexmo', '2factor', 'msg91', 'alphanet_sms', 'smsmisr', 'whysms'] as $gateway) {
                if ($module != $gateway) {
                    $keep = Setting::where(['key_name' => $gateway, 'settings_type' => 'sms_config'])->first();
                    if (isset($keep)) {
                        $hold = is_array($keep->live_values)
                            ? $keep->live_values
                            : json_decode($keep->live_values ?? '{}', true);
