...
class CustomerAuthController extends Controller
{
    public function verify_otp(Request $request)
    {
        ...
        if (env('APP_MODE') == 'test' && $request->otp == '123456') {
    ...
    public function register(Request $request)
    {
        ...
        if(isset($login_settings['phone_verification_status']) && $login_settings['phone_verification_status'] == 1){
            ...
            if(!$firebase_otp_verification){
                ...
                if($published_status == 1){
                    ...
                    // Fallback to core SMS module if addon gateway fails/not configured.
                    if($response !== 'success'){
        ...
        $statusCode = ((int)$phone === 0) ? 470 : 200;
        return response()->json([
            'token' => $token,
    ...
    private function manual_login($request_data){
        ...
        if (auth()->attempt($data)) {
            ...
            $phoneVerificationStatus = (int)(BusinessSetting::where('key', 'phone_verification_status')->first()?->value ?? 0);
            if ($phoneVerificationStatus === 1 && (int)$user->is_phone_verified !== 1) {
                // Force resend OTP on manual-login verification required flow.
                $verification_check = $this->verification_check(['phone' => $user->phone], true);
                ...
                return response()->json([
                    'errors' => [
                        [
                            ...
                            'message' => translate('messages.phone_number_verification_is_required'),
                        ]
                    ],
                    'phone' => $user->phone,
                    'is_phone_verified' => 0,
    ...
    private function verification_check($request_data, $forceResend = false){
        $firebase_otp_verification = BusinessSetting::where('key', 'firebase_otp_verification')->first()?->value??0;
        if(!$firebase_otp_verification)
        {
