...
class CustomerAuthController extends Controller
{
    public function verify_phone_or_email(Request $request)
    {
        ...
        \Log::info('=================== VERIFY PHONE/EMAIL START ===================');
        ...
        $validator = Validator::make($request->all(), [
            ...
            'phone' => 'required_without:mobile|min:9|max:14', // Changed from required_if to required_without
            'mobile' => 'required_without:phone|min:9|max:14', // Support mobile field
        ...
        if($phone){
            ...
            \Log::info('Found user?: ' . ($user ? 'Yes' : 'No'));
        }
        ...
        // Simplified: Always use phone verification logic
        if($user) // if($user && $request->login_type== 'manual')
        {
            ...
            if(env('APP_MODE')=='test')
            {
                if($request['otp']=="123456")
                {
                    ...
                    return response()->json(['token' => isset($token)?$token:$temporaryToken, 'is_phone_verified'=>1, 'is_email_verified'=>1, 'is_personal_info' => $is_personal_info, 'is_exist_user' =>null, 'login_type' => 'otp', 'email' => $user_email], 200); // 'login_type' => $request->login_type
                }
                return response()->json([
            ...
            //     ])->first();
            // }elseif ($request->verification_type == 'phone'){
            //     \Log::info('Checking OTP for normalized phone: ' . $normalized_phone);
            //     \Log::info('OTP provided: ' . $request['otp']);
            //     $data = DB::table('phone_verifications')->where([
            //         'phone' => $normalized_phone,
            //         'token' => $request['otp'],
            //     ])->first();
              
            //     \Log::info('Found verification data?: ' . ($data ? 'Yes' : 'No'));
            //     if ($data) {
            //         \Log::info('Stored token: ' . $data->token);
            //     }
            // }
            \Log::info('Checking OTP for normalized phone: ' . $normalized_phone);
            \Log::info('OTP provided: ' . $request['otp']);
        ...
        // Simplified: Always use OTP login logic
        {
            \Log::info('=================== OTP VERIFICATION (No User) START ===================');
            \Log::info('Normalized phone: ' . $normalized_phone);
            \Log::info('OTP provided: ' . $request['otp']);
            
            // Check all OTPs for this phone for debugging
            $all_otps = DB::table('phone_verifications')->where('phone', $normalized_phone)->get();
            \Log::info('All OTPs in DB for this phone: ' . json_encode($all_otps));
            ...
            \Log::info('Found OTP match?: ' . ($data ? 'Yes' : 'No'));
            ...
            if($data){
                if($user && $user->is_phone_verified == 0 && $user->is_from_pos == 0){
                    ...
                    $user->password = bcrypt($normalized_phone);
                    $user->is_phone_verified = 1;
    ...
    public function register(Request $request)
    {
        ...
        $normalized_phone = $this->normalizePhone($request->phone);
        ...
        $phone = 1;
        $mail = 1;
        $otp = null; // Initialize OTP variable
...
$whySMS = new WhySMSService();
$otp = rand(1000, 9999); // إنشاء OTP عشوائي 4 أرقام

$smsResult = $whySMS->sendOTP($normalized_phone, $otp, 'register');
    ...
    return response()->json([
        'errors' => [['code' => 'otp', 'message' => translate('messages.failed_to_send_sms')]]
              ...
              DB::table('phone_verifications')->updateOrInsert(['phone' => $normalized_phone],
                ...
                // SMS sending commented out - using static OTP instead
                // $published_status = 0;
                ...
                // SMS error check commented out - static OTP is always available
...
$smsResult = $whySMS->sendOTP($normalized_phone, $otp);
...
if($smsResult !== 'success') {
    return response()->json([
        ...
        return response()->json(['token' => $token, 'is_phone_verified'=>$phone, 'is_email_verified'=>$mail, 'is_personal_info' => 1, 'is_exist_user' =>null, 'login_type' => 'manual', 'email' => $user_email, 'otp' => $otp], 200);
    }
    ...
    private function manual_login($request_data){
        ...
        if($request_data['field_type'] == 'email'){
            ...
            // Normalize phone number before login attempt
            $normalized_phone = $this->normalizePhone($request_data['email_or_phone']);
            $data = [
                'phone' => $normalized_phone,
                'password' => $request_data['password'],
            ];
    ...
    {
        \Log::info('=================== PHONE NORMALIZATION START ===================');
        \Log::info('Input phone number: ' . $phone);
        
        // Remove any + symbol
        $phone = str_replace('+', '', $phone);
        \Log::info('Removed + symbol, now: ' . $phone);
        
        // If starts with 200, remove the extra 0
        if (preg_match('/^200/', $phone)) {
            $phone = '20' . substr($phone, 3);
            \Log::info('Removed extra 0 after 200, now: ' . $phone);
        }
        
        // Add + prefix back
        $phone = '+' . $phone;
        \Log::info('Final normalized phone: ' . $phone);
        \Log::info('=================== PHONE NORMALIZATION END ===================');
