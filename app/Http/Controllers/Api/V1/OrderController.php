                ...
                if ($c['item_type'] === 'App\Models\ItemCampaign' || $c['item_type'] === 'AppModelsItemCampaign') {
                    ...
                    if ($product) {
                        ...
                        if ($product->module->module_type == 'food' && $product->food_variations) {
                            ...
                            // إدارة المخزون: لو الموديول مفعّل فيه stock أو المنتج منيو مشترك، فعّل خصم المخزون
                            if (config('module.' . $product->module->module_type)['stock'] || $product->is_shared_menu) {
                                if ($c['quantity'] > $stock) {
                                    return response()->json([
                                        'errors' => [
                    ...
                    if ($product) {
                        ...
                        $productStoreOk = $product->is_shared_menu
                            ? \App\Models\StoreItemStock::where('item_id', $product->id)->where('store_id', $order->store_id)->exists()
                        ...
                        if ($product->module->module_type == 'food' && $product->food_variations) {
                            ...
                            // خصم المخزون: لمنتج food مع variations كمان نضيفه لـ product_data لو موديول stock أو منيو مشترك
                            if (config('module.' . $product->module->module_type)['stock'] || $product->is_shared_menu) {
                                $stock = $product->getStockForStore($order->store_id);
                                if ($c['quantity'] > $stock) {
                                    return response()->json([
                                        'errors' => [
                                ...
                                $product_data[] = [
                                    'item' => clone $product,
                                    'quantity' => $c['quantity'],
                                    'variant' => count($c['variation']) > 0 ? (isset($c['variation'][0]['type']) ? $c['variation'][0]['type'] : null) : null
                                ];
                            }
                            $product->tax = $store->tax;
                            $product = Helpers::product_data_formatting($product, false, false, app()->getLocale());
                            ...
                            $product->tax = $store->tax;
                            $product->setAttribute('context_store_id', $order->store_id);
                            $product = Helpers::product_data_formatting($product, false, false, app()->getLocale());
                            $addon_data = Helpers::calculate_addon_price(\App\Models\AddOn::whereIn('id', $c['add_on_ids'])->get(), $c['add_on_qtys']);
                            $product_discount = Helpers::product_discount_calculate($product, $price, $store);
                            $or_d = [
            ...
            if ($request->order_type !== 'parcel') {
                ...
                if (count($product_data) > 0) {
                    foreach ($product_data as $item) {
                        ProductLogic::update_stock($item['item'], $item['quantity'], $item['variant'], $order->store_id)->save();
        ...
        if (!$order) {
            ...
            if (config('module.' . $order->module->module_type)['stock']) {
                foreach ($order->details as $detail) {
                    ...
                    ProductLogic::update_stock($item, -$detail->quantity, count($variant) ? $variant[0]['type'] : null, $order->store_id)->save();
                }
            }
