...
class DeliverymanController extends Controller
{
    ...
    public function get_profile(Request $request)
    {
        ...
        // Count today's delivered orders only (based on delivered date, not created_at)
        $dm['todays_order_count'] = (integer)\App\Models\Order::where('delivery_man_id', $dm->id)
            ->where('order_status', 'delivered')
            ->whereDate('delivered', now())
            ->count();
        ...
        // Today's earnings from completed orders only (without tips)
        // Calculate earnings from OrderTransaction records created today where the related order is delivered
        $todays_completed_earning = \App\Models\OrderTransaction::where('delivery_man_id', $dm->id)
            ->whereDate('created_at', now())
            ->whereHas('order', function($query) {
                $query->where('order_status', 'delivered');
            })
            ->sum('original_delivery_charge');
        $dm['todays_completed_orders_earning'] = (float)($todays_completed_earning ?? 0);

        // Average earnings per completed order (without tips)
        // Calculate average from TODAY'S completed orders only
        $todays_completed_transactions = \App\Models\OrderTransaction::where('delivery_man_id', $dm->id)
            ->whereDate('created_at', now())
            ->whereHas('order', function($query) {
                $query->where('order_status', 'delivered');
            })
        ...
        $todays_completed_orders_count = $todays_completed_transactions->count();
        $todays_total_earning = (float)$todays_completed_transactions->sum('original_delivery_charge');
        $dm['avg_completed_order_earning'] = $todays_completed_orders_count > 0 ? (float)($todays_total_earning / $todays_completed_orders_count) : 0;
        ...
        // Calculate total earning from all delivered orders
        $show_dm_total_earning = BusinessSetting::where('key', 'show_dm_total_earning')->first()?->value ?? 0;
        if ($show_dm_total_earning == 1) {
            // Calculate total earning from all delivered orders (not just wallet)
            // Get all delivered orders for this delivery man
            $delivered_orders = \App\Models\Order::where('delivery_man_id', $dm->id)
                ->where('order_status', 'delivered')
                ->with('transaction')
                ->get();
            
            $total_earning_from_delivered = 0;
            $total_tips_from_delivered = 0;
            
            foreach ($delivered_orders as $order) {
                if ($order->transaction) {
                    $total_earning_from_delivered += (float)($order->transaction->original_delivery_charge ?? 0);
                    $total_tips_from_delivered += (float)($order->transaction->dm_tips ?? 0);
                }
            }
            
            $dm['total_earning'] = (float) ($total_earning_from_delivered + $total_tips_from_delivered);
        } else {
            // Use wallet total_earning if setting is disabled
        ...
        // Calculate Performance (on-time delivery rate)
        $dm_performance_tracking = BusinessSetting::where('key', 'dm_performance_tracking')->first()?->value ?? 0;
        $dm['performance'] = null;
        if ($dm_performance_tracking == 1) {
            // Get all delivered orders for this delivery man
            $delivered_orders = \App\Models\Order::where('delivery_man_id', $dm->id)
                ->where('order_status', 'delivered')
            ...
            foreach ($delivered_orders as $order) {
                // Skip if order doesn't have delivered time
                if (!$order->delivered) {
                    continue;
                }
                
                // Skip if order doesn't have store or delivery_time
                if (!$order->store || !$order->store->delivery_time) {
                    continue;
                }
                
                // Calculate expected delivery time
                // If order is scheduled, use schedule_at, otherwise use created_at
                $order_start_time = $order->schedule_at && $order->scheduled == 1 
                    ? \Carbon\Carbon::parse($order->schedule_at) 
                    : \Carbon\Carbon::parse($order->created_at);
                
                // Get delivery time from store (expected delivery time)
                // Parse delivery_time format: "30-45 min" or "1-2 hours" or "30-45 minutes"
                $delivery_time = $order->store->delivery_time;
                $delivery_time_minutes = null;
                
                // Extract the maximum time from range (e.g., "30-45 min" -> 45)
                if (preg_match('/(\d+)\s*-\s*(\d+)/', $delivery_time, $matches)) {
                    $max_time = (int)$matches[2]; // Use maximum time for comparison
                    // Convert to minutes if hours
                    if (stripos($delivery_time, 'hour') !== false) {
                        $delivery_time_minutes = $max_time * 60;
                    } else {
                        $delivery_time_minutes = $max_time;
                    }
                } elseif (preg_match('/(\d+)/', $delivery_time, $matches)) {
                    // Single number format
                    $time_value = (int)$matches[1];
                    if (stripos($delivery_time, 'hour') !== false) {
                        $delivery_time_minutes = $time_value * 60;
                    } else {
                        $delivery_time_minutes = $time_value;
                    }
                }
                
                // Skip if we can't determine delivery time
                if (!$delivery_time_minutes) {
                    continue;
                }
                
                // Expected delivery time = order start time + delivery time (in minutes)
                $expected_delivery_time = $order_start_time->copy()->addMinutes($delivery_time_minutes);
                
                // Actual delivery time (delivered)
                $actual_delivery_time = \Carbon\Carbon::parse($order->delivered);
                
                // Count this order
                $total_delivered_orders++;
                
                // Check if delivered on time (delivered <= expected)
            ...
            if ($total_delivered_orders > 0) {
                $dm['performance'] = (float)round(($on_time_orders / $total_delivered_orders) * 100, 2);
            } else {
                $dm['performance'] = 0.0;
            }
        }
