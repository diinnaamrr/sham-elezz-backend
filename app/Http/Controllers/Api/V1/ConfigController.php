...
//            'landing_page_settings_full_url'=> $data['web_app_landing_page_settings_storage']??'public',
    ...
    public function force_update_user()
    {
        $keys = [
            'user_app_maintenance_mode', 'user_app_maintenance_message',
            'user_android_latest_version', 'user_android_force_update', 'user_android_blocked_versions',
            'user_ios_latest_version', 'user_ios_force_update', 'user_ios_blocked_versions',
            'app_minimum_version_android', 'app_url_android',
            'app_minimum_version_ios', 'app_url_ios'
        ];

        $settings = array_column(BusinessSetting::whereIn('key', $keys)->get()->toArray(), 'value', 'key');

        return response()->json([
            'success' => true,
            'data' => [
                'maintenance' => [
                    'enabled' => (int)($settings['user_app_maintenance_mode'] ?? 0),
                    'message' => $settings['user_app_maintenance_message'] ?? ''
                ],
                'android' => [
                    'minimum_version' => $settings['app_minimum_version_android'] ?? '1.0.0',
                    'latest_version' => $settings['user_android_latest_version'] ?? '1.0.0',
                    'force_update' => (int)($settings['user_android_force_update'] ?? 0),
                    'blocked_versions' => json_decode($settings['user_android_blocked_versions'] ?? '[]', true),
                    'update_url' => $settings['app_url_android'] ?? '',
                ],
                'ios' => [
                    'minimum_version' => $settings['app_minimum_version_ios'] ?? '1.0.0',
                    'latest_version' => $settings['user_ios_latest_version'] ?? '1.0.0',
                    'force_update' => (int)($settings['user_ios_force_update'] ?? 0),
                    'blocked_versions' => json_decode($settings['user_ios_blocked_versions'] ?? '[]', true),
                    'update_url' => $settings['app_url_ios'] ?? '',
                ]
            ]
        ]);
    }

    public function force_update_driver()
    {
        $keys = [
            'deliveryman_app_maintenance_mode', 'deliveryman_app_maintenance_message',
            'deliveryman_android_latest_version', 'deliveryman_android_force_update', 'deliveryman_android_blocked_versions',
            'deliveryman_ios_latest_version', 'deliveryman_ios_force_update', 'deliveryman_ios_blocked_versions',
            'app_minimum_version_android_deliveryman', 'app_url_android_deliveryman',
            'app_minimum_version_ios_deliveryman', 'app_url_ios_deliveryman'
        ];
        ...
        return response()->json([
            ...
            'data' => [
                'maintenance' => [
                    'enabled' => (int)($settings['deliveryman_app_maintenance_mode'] ?? 0),
                    'message' => $settings['deliveryman_app_maintenance_message'] ?? ''
                ],
                'android' => [
                    'minimum_version' => $settings['app_minimum_version_android_deliveryman'] ?? '1.0.0',
                    'latest_version' => $settings['deliveryman_android_latest_version'] ?? '1.0.0',
                    'force_update' => (int)($settings['deliveryman_android_force_update'] ?? 0),
                    'blocked_versions' => json_decode($settings['deliveryman_android_blocked_versions'] ?? '[]', true),
                    'update_url' => $settings['app_url_android_deliveryman'] ?? '',
                ],
                'ios' => [
                    'minimum_version' => $settings['app_minimum_version_ios_deliveryman'] ?? '1.0.0',
                    'latest_version' => $settings['deliveryman_ios_latest_version'] ?? '1.0.0',
                    'force_update' => (int)($settings['deliveryman_ios_force_update'] ?? 0),
                    'blocked_versions' => json_decode($settings['deliveryman_ios_blocked_versions'] ?? '[]', true),
                    'update_url' => $settings['app_url_ios_deliveryman'] ?? '',
                ]
            ]
        ]);
    }

    public function force_update_store()
    {
        $settings = \App\Models\BusinessSetting::whereIn('key', [
            'store_android_latest_version', 'store_android_force_update', 'store_android_blocked_versions',
            'store_ios_latest_version', 'store_ios_force_update', 'store_ios_blocked_versions',
            'app_url_android_store', 'app_url_ios_store',
            'store_app_maintenance_mode', 'store_app_maintenance_message',
            'app_minimum_version_android_store', 'app_minimum_version_ios_store'
        ...
        return response()->json([
            ...
            'data' => [
                'maintenance' => [
                    'enabled' => (int)($settings['store_app_maintenance_mode'] ?? 0),
                    'message' => $settings['store_app_maintenance_message'] ?? ''
                ],
                'android' => [
                    'minimum_version' => $settings['app_minimum_version_android_store'] ?? '1.0.0',
                    'latest_version' => $settings['store_android_latest_version'] ?? '1.0.0',
                    'force_update' => (int)($settings['store_android_force_update'] ?? 0),
                    'blocked_versions' => json_decode($settings['store_android_blocked_versions'] ?? '[]', true),
                    'update_url' => $settings['app_url_android_store'] ?? ''
                ],
                'ios' => [
                    'minimum_version' => $settings['app_minimum_version_ios_store'] ?? '1.0.0',
                    'latest_version' => $settings['store_ios_latest_version'] ?? '1.0.0',
                    'force_update' => (int)($settings['store_ios_force_update'] ?? 0),
                    'blocked_versions' => json_decode($settings['store_ios_blocked_versions'] ?? '[]', true),
                    'update_url' => $settings['app_url_ios_store'] ?? ''
                ]
            ]
        ]);
    }
