...
class ItemController extends Controller
{
    ...
    public function get_latest_products(Request $request)
    {
        ...
        if ($contextStoreId) {
            foreach ($items['products'] as $p) {
                if (($p->is_shared_menu ?? false) && !$p->getAttribute('context_store_id')) {
                    $p->setAttribute('context_store_id', $contextStoreId);
                }
            }
        }
    ...
    public function item_or_store_search(Request $request){
        ...
        ->where(function ($q) use ($key) {
            ...
            //     });
            // });
            // $q->orWhereHas('ecommerce_item_details.brand',function($query)use($key){
            //     $query->where(function($q)use($key){
            //         foreach ($key as $value) {
            //             $q->where('name', 'like', "%{$value}%");
            //         };
            //     });
            // });
            // $q->orWhereHas('pharmacy_item_details.common_condition',function($query)use($key){
            //     $query->where(function($q)use($key){
            //         foreach ($key as $value) {
            //             $q->where('name', 'like', "%{$value}%");
            //         };
