...
class Item extends Model
{
    ...
    public function getNameAttribute($value){
        if (count($this->translations) > 0) {
            foreach ($this->translations as $translation) {
                if ($translation['key'] == 'name' && (string) $translation['value'] !== '') {
    ...
    /** مخزون المطاعم للمنتج المشترك (يُستخدم عندما is_shared_menu = true) */
    public function storeStocks()
    {
        return $this->hasMany(StoreItemStock::class);
    }

    /**
     * رصيد المنتج لمطعم معيّن: من items.stock لو المنتج خاص بمطعم، أو من store_item_stock لو منيو مشترك
     */
    public function getStockForStore(?int $storeId): int
    {
        if ($this->is_shared_menu && $storeId) {
    ...
    /**
     * Scope: منتجات متاحة لمطعم معيّن (خاصة بالمطعم أو منيو مشترك ومسجّل للمطعم)
     */
    ...
    /** فلترة حسب الرصيد الفعلي لمطعم (للمنتجات العادية أو المشتركة) */
    public function scopeInStockForStore($query, ?int $storeId)
    {
        if (!$storeId) {
            return $query->where('items.stock', '>', 0);
        }
        return $query->where(function ($q) use ($storeId) {
            $q->where(function ($q2) {
                $q2->where('items.is_shared_menu', false)->where('items.stock', '>', 0);
            })->orWhere(function ($q2) use ($storeId) {
                $q2->where('items.is_shared_menu', true)
                    ->whereExists(function ($sub) use ($storeId) {
                        $sub->select(\Illuminate\Support\Facades\DB::raw(1))
                            ...
                            ->whereColumn('store_item_stock.item_id', 'items.id')
                            ->where('store_item_stock.store_id', $storeId)
                            ->where('store_item_stock.stock', '>', 0);
                    });
            });
        });
    }
