        ...
        <form action="javascript:" method="post" id="product_form" enctype="multipart/form-data">
            @csrf
                @if (request()->product_gellary  == 1)
                    @php
                        $route = route('admin.item.store', ['product_gellary' => request()->product_gellary]);
                        $product->price = 0;
                    @endphp
                @else
                    @php
                        $route = route('admin.item.update', [isset($temp_product) && $temp_product == 1 ? $product['item_id'] : $product['id']]);
                    @endphp
                @endif
            ...
            <div class="row g-2">
                                ...
                                <div class="flex-grow-1 mx-auto">
                                    ...
                                    <div class="d-flex flex-wrap __gap-12px __new-coba" id="coba">
                                        ...
                                        @foreach($product->images as $key => $photo)
                                            @php
                                                $photo = is_array($photo) ? $photo : ['img' => $photo, 'storage' => 'public'];
                                            @endphp
                ...
                <div class="col-md-12">
                    <div class="card shadow--card-2 border-0">
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                <div class="col-sm-6 col-lg-3" id="store_id_wrapper">
                                    <div class="form-group mb-0">
                                        <label class="input-label" for="store_id">{{ translate('messages.store') }}  <span class="form-label-secondary text-danger store-required-asterisk"
                                            data-toggle="tooltip" data-placement="right"
                                            data-original-title="{{ translate('messages.Required.')}}"> *
                                            </span><span
                                        ...
                                        <select name="store_id"
                                            ...
                                            title="{{ translate('messages.select_store') }}" {{ (isset(request()->product_gellary) == false && !($product->is_shared_menu ?? false)) ? 'required' : '' }}
                                ...
                                <div class="col-sm-6 col-lg-3 d-flex align-items-end">
                                    <div class="form-group mb-0">
                                        <div class="form-check form-check-inline mt-4">
                                            <input type="checkbox" name="is_shared_menu" id="is_shared_menu" value="1" class="form-check-input"
                                                {{ ($product->is_shared_menu ?? false) ? 'checked' : '' }}>
                                            <label class="form-check-label input-label" for="is_shared_menu">
                                                {{ translate('messages.Shared menu (product in all restaurants)') }}
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6 col-lg-3 d-none" id="shared_menu_stock_wrapper">
                                    <div class="form-group mb-0">
                                        <label class="input-label" for="shared_menu_default_stock">{{ translate('messages.Default stock for all restaurants') }} <span class="text-danger">*</span></label>
                                        @php
                                            $shared_menu_stock_value = 0;
                                            if ($product instanceof \App\Models\Item && ($product->is_shared_menu ?? false) && $product->storeStocks->isNotEmpty()) {
                                                $shared_menu_stock_value = $product->storeStocks->first()->stock;
                                            }
                                        @endphp
                                        <input type="number" name="shared_menu_default_stock" id="shared_menu_default_stock" class="form-control" min="0" value="{{ $shared_menu_stock_value }}" placeholder="0">
                                        <small class="text-muted">{{ translate('messages.Product will be added to all restaurants with this stock') }}</small>
                                    </div>
                                </div>
                                ...
                                @if(Config::get('module.current_module_type') == 'grocery' || Config::get('module.current_module_type') == 'food')
                                    @if (isset($temp_product) && $temp_product == 1 )
                                        @php
                                            $product_nutritions = \App\Models\Nutrition::whereIn('id', json_decode($product?->nutrition_ids))->pluck('id');
                                            $product_allergies = \App\Models\Allergy::whereIn('id', json_decode($product?->allergy_ids))->pluck('id');
                                        @endphp
                                    @else
                                        @php
                                            $product_nutritions = $product->nutritions->pluck('id');
                                            $product_allergies = $product->allergies->pluck('id');
                                        @endphp
                                    @endif
                    ...
                    <div class="card-body pb-0">
                        <div class="row g-2">
                            <div class="col-12">
                                ...
                                <div class="form-group">
                                    @php
                                        $tags = \App\Models\Tag::whereIn('id', json_decode($product?->tag_ids))->get('tag');
                                    @endphp
...
<script>
    ...
    function add_new_row_button(data) {
     ...
     function toggleSharedMenuUi() {
         let isShared = $('#is_shared_menu').is(':checked');
         if (isShared) {
             $('#shared_menu_stock_wrapper').removeClass('d-none');
             $('#store_id_wrapper').addClass('d-none');
             $('#store_id').removeAttr('required').val('');
             $('.store-required-asterisk').addClass('d-none');
             $('#shared_menu_default_stock').attr('required', true);
         } else {
             $('#shared_menu_stock_wrapper').addClass('d-none');
             $('#store_id_wrapper').removeClass('d-none');
             $('#store_id').attr('required', true);
             $('.store-required-asterisk').removeClass('d-none');
             $('#shared_menu_default_stock').removeAttr('required');
         }
     }
     $('#is_shared_menu').on('change', toggleSharedMenuUi);
     toggleSharedMenuUi();
