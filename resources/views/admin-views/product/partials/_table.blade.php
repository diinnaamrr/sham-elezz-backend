@foreach ($items as $key => $item)
    <tr>
        ...
        <td>
            <a class="media align-items-center" href="{{ route('admin.item.view', [$item['id']]) }}">
                ...
                data-onerror-image="{{ asset('assets/admin/img/160x160/img2.jpg') }}"
                    alt="{{ $item->getRawOriginal('name') ?? $item->name }} image">
        ...
        <td>
            {{ Str::limit($item->store ? $item->store->name : (($item->is_shared_menu ?? false) ? translate('messages.all_stores_shared') : translate('messages.store deleted!')), 20, '...') }}
        </td>
        <td>
